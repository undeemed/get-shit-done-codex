...
- **BREAKING:** Removed prompts mode entirely — all commands now use `$gsd-*` skill notation only
- Removed `--codex-mode` installer flag and interactive mode picker
- Converted all `/gsd:` and `/prompts:gsd-` references to `$gsd-` across all source files (69 files, -1730 lines)
- Simplified installer: no mode selection, always installs `skills/gsd-*/SKILL.md`
- Updated USER-GUIDE.md and README.md to reflect skills-only command surface

### Removed

- `installPrompts()` function and prompt alias generation
- `promptCodexMode()` interactive mode picker
- `convertSkillRefsToPromptRefs()` conversion utility
- `--codex-mode` flag from CLI and help text
- Prompts mode tests and hybrid mode validation
- `new-project.md.bak` stale backup file

### Migration

- Existing `prompts/` directories are detected as legacy and cleaned up via `--migrate`
- `--verify` warns about legacy prompt files; `--verify --repair --migrate` removes them
...
- Phase directory matching now handles both zero-padded (05-_) and unpadded (5-_) folder names
